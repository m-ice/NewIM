...
The inner PL/pgSQL exception block rolls back ALL changes it made, including counter/summary/outbox, on a unique violation. A fresh READ COMMITTED lookup by sender/client identity returns the committed original if one exists. PostgreSQL may report either the primary key or the sender/client constraint when a retry also reuses serverMsgId, so the reported constraint name cannot determine idempotency. Without a matching identity the original unique violation propagates, including unrelated server ID/event ID/sequence collisions. The outer caller transaction still owns COMMIT. A caller rollback, backend termination or process crash before commit cannot leave a partial persisted result. Lock order is conversation row before message/summary/outbox; future multi-conversation operations must order conversation IDs deterministically. Adding permission/membership locks requires review of the resulting order.
