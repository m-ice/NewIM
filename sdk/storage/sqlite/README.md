...
4. Call `rebuild(root, account, limits)` only as an explicit recovery decision. With a missing or legacy generic initialized marker, the original account must be readable from supported quarantine metadata; another account is rejected, and unreadable identity returns RecoveryRequired before active files are created. Unbound incomplete first-creation remnants cannot be rebuilt in place; preserve them and explicitly select a separate new directory if needed. The new DB has a new instance and reset cursor. The original quarantine remains. Interrupted rebuild can resume from the durable marker.
