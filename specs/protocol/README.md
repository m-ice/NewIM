...
提供 Go 与 Rust 的 `Decode/decode`、`Encode/encode`、`Validate/validate`，用于服务端和 SDK 共用的消息契约。支持文本 v1；未知消息类型或未来文本 schema 保留 payload 并报告 unsupported。解码成功仅表示格式有效，不代表消息已持久化、已送达或发送者有权限。
...
`payload` 为对象，文本 v1 的 `text` 必填且为 1..16384 个 UTF-8 字节；不裁剪或归一化文本。未知 envelope 字段可丢弃；未知 payload 字段保留。未知类型/未来文本版本不执行已知文本校验，调用方必须检查 `Supported()/supported()`。未知 protocolVersion 拒绝。
