...
func (r *LocalSecretResolver) Resolve(ctx context.Context, material SecretMaterial) ([]byte, error) {
	if r == nil || r.aead == nil || ctx == nil || material.DestinationID == "" || material.Revision <= 0 || material.KeyID == "" || len(material.Nonce) != r.aead.NonceSize() || len(material.Ciphertext) < r.aead.Overhead()+1 {
...
func SealSecret(aead cipher.AEAD, material SecretMaterial, secret []byte) ([]byte, error) {
	if aead == nil || len(secret) < 32 || len(material.Nonce) != aead.NonceSize() || material.DestinationID == "" || material.Revision <= 0 || material.KeyID == "" {
		return nil, Fail(CodeInvalidConfig)
	}
